...
public class SysUserMajorServiceImpl extends ServiceImpl<SysUserMajorMapper, Major> implements SysUserMajorService {
    ...
    public boolean checkModifyMaxCountAuth(Long operatorId, Long targetUserId, Long majorId) {
        ...
        if (operator == null || operator.getIsDeleted() == 1) {
            throw new BusinessException(ResultCode.RESOURCE_NOT_FOUND, "操作人不存在或已删除");
        }

        // 2. 角色判断：
        // 情况1：管理员/超级管理员 → 拥有所有专业、所有教师的上限修改权限
        if (operator.getUserType().equals("ADMIN") || operator.getUserType().equals("SUPER_ADMIN")) {
            return true;
        }
        ...
        if (operator.getUserType().equals("TEACHER")) {
            ...
            throw new BusinessException(ResultCode.PERMISSION_NO_ACCESS, "权限不足：教师角色无法修改指导任务上限，请联系管理员操作");
        }

        // 情况3：学生（user_type=1）或其他角色→ 无任何权限
        throw new BusinessException(ResultCode.PERMISSION_ADMIN_ONLY, "权限不足：仅管理员可修改指导任务上限");
    }
    ...
    public Major modifyAdvisorMaxCount(Long operatorId, Long targetUserId, Long majorId, Integer newMaxCount) {
        ...
        if (targetTeacher == null || targetTeacher.getIsDeleted() == 1) {
            throw new BusinessException(ResultCode.RESOURCE_NOT_FOUND, "目标教师不存在或已删除");
        }
        if (!targetTeacher.getUserType().equals("TEACHER")) {
            throw new BusinessException(ResultCode.PARAM_ERROR, "目标用户不是教师角色，无需配置指导任务上限");
        }
        ...
        if (userMajor == null) {
            throw new BusinessException(ResultCode.RESOURCE_NOT_FOUND, "目标教师未关联该专业（专业ID：" + majorId + "），无法修改上限");
        }

        // 4. 校验新上限的合法性（1-50之间，避免不合理值）
        if (newMaxCount < 1 || newMaxCount > 50) {
            throw new BusinessException(ResultCode.PARAM_VALUE_INVALID, "指导任务上限需在1-50之间，请重新输入");
        }

        // 5. 校验新上限是否小于当前指导数（避免设置的上限低于已分配任务数）
        Integer currentCount = userMajor.getCurrentAdvisorCount();
        if (newMaxCount < currentCount) {
            throw new BusinessException(ResultCode.BUSINESS_NO_TASK_MAX,
