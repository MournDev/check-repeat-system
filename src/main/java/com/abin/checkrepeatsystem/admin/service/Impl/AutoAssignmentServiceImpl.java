...
public class AutoAssignmentServiceImpl implements AutoAssignmentService {
    ...
    public Result<Map<String, Object>> getAssignmentDetail(String id) {
        ...
        if (history == null) {
            throw new BusinessException(ResultCode.RESOURCE_NOT_FOUND, "执行记录不存在");
        }
    ...
    private void validateConfig(AutoAssignmentConfigDTO config) {
        if (config.getMaxLoad() == null || config.getMaxLoad() <= 0) {
            throw new BusinessException(ResultCode.PARAM_ERROR, "最大指导学生数必须大于0");
        }

        int totalWeight = config.getMajorWeight() + config.getInterestWeight() +
                         config.getLoadWeight() + config.getExperienceWeight();
        if (totalWeight != 100) {
            throw new BusinessException(ResultCode.PARAM_ERROR, "权重总和必须等于100");
        }

        if (config.getMinLoadPerTeacher() > config.getMaxLoad()) {
            throw new BusinessException(ResultCode.PARAM_ERROR, "最小指导学生数不能大于最大指导学生数");
        }
    }
    ...
    private List<SysUser> getAvailableTeachers(AutoAssignmentConfigDTO config) {
        ...
        if (config.getExcludeFullTeachers()) {
            // 过滤掉满负荷的教师
            int maxLoad = config.getMaxLoad() != null ? config.getMaxLoad() : 12;
    ...
    private int calculateExperienceYears(SysUser teacher) {
        TeacherInfo teacherInfo = teacherInfoService.getByUserId(teacher.getId());
        if (teacherInfo != null && teacherInfo.getCreateTime() != null) {
            return (int) java.time.Duration.between(
                    teacherInfo.getCreateTime(), java.time.LocalDateTime.now()).toDays() / 365;
        }
        return 5; // 默认5年经验
    }
    ...
    private void assignStudentToTeacher(PaperInfo student, SysUser teacher) {
        ...
        student.setAllocationType(DictConstants.AllocationType.AUTO);
        student.setAllocationStatus(DictConstants.AllocationStatus.PENDING);
        student.setAllocationTime(LocalDateTime.now());
        student.setPaperStatus(DictConstants.PaperStatus.ASSIGNED);
