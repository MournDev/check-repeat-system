...
public class PendingReviewServiceImpl implements PendingReviewService {
    ...
    public Result<Map<String, Object>> recheckPlagiarism(Long teacherId, Long paperId) {
        ...
        if (!paperInfo.getTeacherId().equals(teacherId)) {
            throw new BusinessException(ResultCode.PERMISSION_NO_ACCESS, "无权限操作该论文");
        }

        // 更新论文状态为查重中
        paperInfo.setPaperStatus(PaperStatusEnum.CHECKING.getValue());
    ...
    public Result<Map<String, Object>> contactStudent(Long teacherId, ContactStudentDTO contactDTO) {
        ...
        switch (contactDTO.getMessageType().toLowerCase()) {
            ...
            default:
                throw new BusinessException(ResultCode.PARAM_ERROR, "不支持的消息类型: " + contactDTO.getMessageType());
        }
    }
    ...
    private Result<Map<String, Object>> sendInstantMessage(Long teacherId, ContactStudentDTO contactDTO, Map<String, Object> result) {
        ...
        if (sendResult.isSuccess() && Boolean.TRUE.equals(sendResult.getData())) {
            ...
            throw new BusinessException(ResultCode.SYSTEM_ERROR, "即时消息发送失败: " + sendResult.getMessage());
        }
    }
    ...
    private Result<Map<String, Object>> sendSystemMessage(Long teacherId, ContactStudentDTO contactDTO, Map<String, Object> result) {
        ...
        if (sendResult.isSuccess() && Boolean.TRUE.equals(sendResult.getData())) {
            ...
            throw new BusinessException(ResultCode.SYSTEM_ERROR, "系统消息发送失败: " + sendResult.getMessage());
        }
    }
    ...
    public Result<PaperContentDTO> getPaperContent(Long teacherId, Long paperId) {
        ...
        if (!paperInfo.getTeacherId().equals(teacherId)) {
            throw new BusinessException(ResultCode.PERMISSION_NO_ACCESS, "无权限访问此论文");
        }
    ...
    public Result<Map<String, Object>> delegateReview(Long teacherId, DelegateReviewDTO delegateDTO) {
        ...
        if (!paperInfo.getTeacherId().equals(teacherId)) {
            throw new BusinessException(ResultCode.PERMISSION_NO_ACCESS, "无权限委托此论文");
        }

        // 2. 验证被委托教师
        SysUser delegateTeacher = sysUserMapper.selectById(Long.parseLong(delegateDTO.getDelegateTeacherId()));
        if (delegateTeacher == null) {
            throw new BusinessException(ResultCode.RESOURCE_NOT_FOUND, "被委托教师不存在");
        }
